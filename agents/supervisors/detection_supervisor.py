...
logger = logging.getLogger("DetectionSupervisor")

class DetectionSupervisor(BaseAgent):
    def __init__(self, supervisor_queue):
        super().__init__(
            name="DetectionSupervisor",
            description="Supervises detection and intelligence agents",
            supervisor_queue=supervisor_queue,
            interval_seconds=15
        )
        self.config = SOCConfig()
        self.managed_workers = ["W13_AnomalyDetection", "W19_ThreatFeeds", "W21_IOCEnrichment"]
        self.recent_alerts = []
...
if __name__ == "__main__":
    agent = DetectionSupervisor(supervisor_queue="soc:detection-supervisor")
