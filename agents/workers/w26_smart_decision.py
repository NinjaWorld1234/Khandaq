...
class SmartDecisionAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[SOCConfig] = None) -> None:
        ...
        self._decision_index: str = self._agent_config.get("decision_index", DECISION_INDEX)
        self._tz_offset_hours: int = self._agent_config.get("tz_offset_hours", 0)
    ...
    def collect(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            new_alerts = [
                a for a in alerts
                if a.get("_id", a.get("id", "")) not in self._processed_alert_ids
            ]

            if new_alerts:
                logger.info(
                    "Collected %d new CRITICAL/HIGH alerts for decision (%d filtered as duplicates)",
                    len(new_alerts), len(alerts) - len(new_alerts),
                )
    ...
    def analyze(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for alert in data:
            ...
            severity_str = alert.get("severity", alert.get("alert_severity", "HIGH")).upper()
    ...
    def decide(self, findings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for ctx in findings:
            ...
            rationale = self._build_rationale(ctx, response_level, rationale_parts)
    ...
    def act(self, actions: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for decision in actions:
            ...
            try:
                ...
                logger.error("Failed to execute decision for alert %s: %s", alert_id, exc)
    ...
    def _extract_hostname(alert: Dict[str, Any]) -> str:
        ...
        hostname = (
            alert.get("hostname")
            or alert.get("host", {}).get("name")
            or alert.get("agent", {}).get("name")
            or alert.get("source", {}).get("hostname")
    ...
    def _is_business_hours(self) -> bool:
        ...
        now = datetime.now(timezone.utc) + timedelta(hours=self._tz_offset_hours)
