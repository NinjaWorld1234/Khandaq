...
class ContainerSecurityAgent(BaseAgent):
    ...
    def collect(self) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            return self.os_client.get_events_since(
                index="wazuh-alerts-*", minutes=2, query=query, size=3000,
            )
    ...
    def analyze(self, data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for event in data:
            agent_name = event.get("agent", {}).get("name", "unknown-host")
            rule_desc = event.get("rule", {}).get("description", "")
            full_log = event.get("full_log", event.get("data", {}).get("log", ""))
            docker_action = event.get("data", {}).get("docker", {}).get("Action", "")
            docker_type = event.get("data", {}).get("docker", {}).get("Type", "")
            container_attrs = event.get("data", {}).get("docker", {}).get("Actor", {}).get("Attributes", {})
            container_name = container_attrs.get("name", "unknown")
            image = container_attrs.get("image", event.get("data", {}).get("docker", {}).get("from", ""))
            command = event.get("data", {}).get("command", full_log)
    ...
    def _check_resource_usage(self) -> List[Dict[str, Any]]:
        ...
        try:
            stats = self.os_client.get_events_since(
                index="wazuh-alerts-*", minutes=2, query=query, size=500,
            )
            for stat in stats:
                docker_data = stat.get("data", {}).get("docker", {})
                cpu_pct = float(docker_data.get("cpu_pct", 0))
                mem_pct = float(docker_data.get("mem_pct", 0))
                container = docker_data.get("name", "unknown")
                host = stat.get("agent", {}).get("name", "unknown-host")
    ...
    def decide(self, findings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for finding in findings:
            ...
            if now - self._alerted_cache.get(alert_key, 0.0) < self._alert_cooldown:
                continue
