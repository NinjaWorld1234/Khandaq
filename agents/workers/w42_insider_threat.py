...
class InsiderThreatAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[SOCConfig] = None) -> None:
        ...
        self._risk_ledger: Dict[str, List[Tuple[float, float, str]]] = defaultdict(list)
    ...
    def analyze(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for event in file_events:
            ...
            self._scope_learning_cycles[user] = self._scope_learning_cycles.get(user, 0) + 1
        ...
        for event in download_events:
            ...
            file_name = self._extract(event, "data.win.eventdata.targetFilename") or ""
            # Approximate size from Sysmon if available
            size_str = self._extract(event, "data.win.eventdata.fileSize") or "0"
        ...
        for event in usb_events:
            ...
            device = (self._extract(event, "data.win.eventdata.deviceDescription")
                      or self._extract(event, "data.win.eventdata.className") or "USB Device")
        ...
        for user, count in print_counts.items():
            ...
            baseline = sum(self._print_samples[user]) / max(len(self._print_samples[user]), 1)
        ...
        for event in email_events:
            user = self._extract(event, "data.srcuser") or self._extract(event, "data.win.eventdata.subjectUserName") or ""
        ...
        if findings:
            logger.warning("Detected %d insider threat indicator(s)", len(findings))
        return findings
        ...
        for action in actions:
            if action["type"] == "alert":
                ...
                try:
                    ...
                    logger.error("Failed to log insider threat incident: %s", exc)
